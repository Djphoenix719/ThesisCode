...
def evaluate_invalid(
    ...
    if gpu_count > 1:
        # NOTE DOES NOT WORK, I only trained on 1 GPU so I never bothered debugging
        processes = [
            Process(
                target=worker,
                args=(
...
def worker(
    ...
    """
    Evaluate the queue of models to train using the specified device, then store the results in the cache.
    NOTE: I never correctly got this working multi-GPU, so you'll have to debug it if you want to use it with more than 1 GPU.
    :param eval_queue: Queue of models.
    :param eval_cache: Models already evaluated.
