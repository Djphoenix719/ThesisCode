...
def drop_tournament_fn(
    n_rand: int, n_worst: int
) -> Callable[[List[SpecWrapper]], List[SpecWrapper]]:
    """
    Drop the n worst individuals from amongst k randomly selected candidates.
    :param n_rand: The number of candidates to randomly select.
    :param n_worst: The number of worst candidates to be dropped.
    :return: The remaining individuals.
    """
    ...
    def fn(population: List[SpecWrapper]):
        ...
        candidate_idxs = sel_rand(list(range(len(population))))
        candidate_idxs.sort() # since pop is in ascending order, idxes can be sorted too
        candidate_idxs.reverse() # do in reverse order so we grab the right elements

        for idx in candidate_idxs[-n_worst:]:
            population.pop(idx)
